...
boxplot(dat$faminc_new,
        main = "Arun's Boxplot of Family Income",
        xlab = "Family Income")
```
...
males <- filter(dat, gender == 1,region == 1| region== 2,marstat == 1,newsint == 1)
str(males)
```
...
dat <- dat %>% mutate(tax_scale = CC18_325a + CC18_325d)
dat$tax_scale <- recode(dat$tax_scale, "2" = 2, "3" = 1, "4" = 0)
...
```{r problem9}
dat1 <- group_by(dat, region) %>% summarise(Trump_Approve_Mean = mean(CC18_308a))
dat1
```
...
```{r problem10}
dat3 <- dat %>% filter(investor == 2 & faminc_new >= 5 & faminc_new < 11) %>% summarize("Mean Religion Imp" = mean(pew_religimp), "Median Religion Imp" = median(pew_religimp), "Standard Dev. Religion Imp" = sd(pew_religimp))
dat3
```
...
```{r problem11}
dat4 <- summarize(dat, "Mean" = mean(faminc_new), "Median" = median(faminc_new), "Std Dev." = sd(faminc_new))
...
qplot(pid7, data = dat, geom = "histogram", xlab = "Seven-Point Party ID", ylab = "Count")
```